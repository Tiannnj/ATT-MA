...
class Buffer:
    ...
    def sample(self, indices):
        ...
        rewards = torch.as_tensor(rewards, dtype=torch.float32).reshape(-1, 1).to(
            self.device)  # torch.Size([batch_size]) -> torch.Size([batch_size, 1])
...
class PER_Buffer:
    ...
    def __init__(self, capacity, obs_dim, act_dim, device, alpha=0.6, beta=0.4, beta_increment=0.001, epsilon=0.01):
    ...
    def sample(self, batch_size):
        ...
        sampling_probabilities = np.clip(sampling_probabilities, 1e-10, None)  # 防止出现0概率
