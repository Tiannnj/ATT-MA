...
class Alpha:
    def __init__(self, action_dim, alpha_lr=0.0001, alpha=0.01, requires_grad=False, is_continue=True):
        ...
        self.log_alpha = torch.tensor(np.log(alpha), dtype=torch.float32,
                                      requires_grad=requires_grad)  # We learn log_alpha instead of alpha to ensure that alpha=exp(log_alpha)>0
        ...
        if is_continue:
            ...
            self.target_entropy = 0.6 * (
                -torch.log(torch.tensor(1.0 / action_dim)))  # 参考:https://zhuanlan.zhihu.com/p/566722896
...
class MASAC:  # 先无attention 再加入
    def __init__(self, dim_info, is_continue, actor_lr, critic_lr, buffer_size, device, trick=None):
        ...
        for agent_id, (obs_dim, action_dim) in dim_info.items():
            if self.adaptive_alpha:
                self.alphas[agent_id] = Alpha(action_dim, alpha=0.01, requires_grad=True,
                                              is_continue=is_continue)  # Alpha(action_dim).alpha 才是值
            else:
    ...
    def learn(self, batch_size, gamma, tau):
        ...
        for agent_id, agent in self.agents.items():
            ...
            q1_next_target, q2_next_target = agent.critic_target(next_obs.values(),
                                                                 next_action.values())  # batch_size x 1
            q_next_target = torch.min(q1_next_target, q2_next_target)

            ''' SAC 特有 '0' 参考github 将next_log_pi 求和 来更新critic , '1' MAAC论文 将当前的next_log_pi 用于更新critic '''
            if self.entropy_way_c == '0':
                next_log_pi = torch.stack([next_log_pi[agent_id] for agent_id in self.agents.keys()], dim=1).sum(
                    dim=1)  # batch_size x 3 x 1 -> batch_size x 1
            ...
            if self.entropy_way_a == '0':
                new_log_pi = torch.stack([agent.actor(obs[agent_id])[1] for agent_id, agent in self.agents.items()],
                                         dim=1).sum(dim=1)  # batch_size x 3 x 1 -> batch_size x 1
                entropy = - new_log_pi
            ...
            actor_loss = (- q_pi - self.alphas[
                agent_id].alpha.detach() * entropy).mean()  # 这里alpha一定要加detach(),因为在更新critic时,计算图被丢掉了
            agent.update_actor(actor_loss)

            ## 更新alpha
            '''公式: Lα = E_{s,a ~ D} [-α * log_pi_a(s,a) - α * H] = E_{s,a ~ D} [α * (-log_pi_a(s,a) - H)]'''
            if self.adaptive_alpha:
                alpha_loss = (self.alphas[agent_id].alpha * (
                            entropy - self.alphas[agent_id].target_entropy).detach()).mean()
...
if __name__ == '__main__':
    ...
    while episode_num < args.max_episodes:
        ...
        done_bool = {agent_id: done[agent_id] if not truncated[agent_id] else False for agent_id in
                     env_agents}  ### truncated 为超过最大步数
