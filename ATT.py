...
class MLPNetworkWithAttention(nn.Module):
    ...
    def forward(self, x, agent_id, agents):
        ...
        out_obs_list = [F.relu(self.fc_obs(x[:, :12])), F.relu(self.fc_obs(x[:, 12:24])),
                        F.relu(self.fc_obs(x[:, 24:36]))]
        # out_obs_list shape: [(batch_size, hidden_dim), ...] #即 batch_size * hidden_dim * agent_count

        out_action_list = [F.relu(self.fc_action(x[:, 36:37])), F.relu(self.fc_action(x[:, 37:38])),
                           F.relu(self.fc_action(x[:, 38:39]))]
...
class Attention_ATT_2(nn.Module):
    ...
    def forward(self, encoder_input, decoder_input):
        ...
        encoder_heads = torch.stack([F.relu(head(encoder_h)) for head in self.fc_encoder_heads],
                                    dim=0)  # head_count * batch_size * hidden_dim

        decoder_H = F.relu(self.fc_decoder_input(decoder_input))  # batch_size * hidden_dim

        ''' enocde_heads 用作键值对 decoder_H 用作查询 '''
        scores = torch.sum(torch.mul(encoder_heads, decoder_H),
                           dim=2)  # scores shape: (head_count, batch_size) <- before sum (head_count, batch_size, hidden_dim)

        attention_weights = F.softmax(scores.permute(1, 0), dim=1).unsqueeze(2)  # batch_size x head_count x 1

        contextual_vector = torch.matmul(encoder_heads.permute(1, 2, 0),
                                         attention_weights).squeeze()  # batch_size x hidden_dim
...
class MLPNetworkWithAttention_2(nn.Module):
    ...
    def forward(self, x, agent_id, agents):
        ...
        encoder_input = self.fc1(torch.cat((x[:, :self.all_obs_dim], action_list[agent_id_index]),
                                           1))  # batch_size * 37 -> batch_size * hidden_dim
        decoder_input = self.fc2(torch.cat((action_list[:agent_id_index] + action_list[agent_id_index + 1:]),
                                           1))  # batch_size * 2 -> batch_size * hidden_dim
...
class ATT_critic(nn.Module):
    ...
    def forward(self, s, a):
        ...
        decoder_input = self.decoder_input_fc(
            torch.cat([a[:, self.a_d[i]:self.a_d[i + 1]] for i in range(self.agent_n) if i != self.id],
                      dim=1))  # 其余智能体动作 # 如action_dim 一致 则batch_size * (action_dim * (agent_count - 1))
...
class ATT_critic_raw(nn.Module):
    ...
    def __init__(self, dim_info, hidden_dim=128, head_count=4):
        ...
        # 注意力相关
        ''' 这里是每个智能体给一个attention模块'''
        self.attention_modules = Attention_ATT(hidden_dim * (self.agent_n + 1), hidden_dim * (self.agent_n - 1),
                                               hidden_dim, head_count)
        self.fc_obs = {i: nn.Linear(obs_act[0], hidden_dim) for i, obs_act in
                       zip(range(self.agent_n), dim_info.values())}
        self.fc_action = {i: nn.Linear(obs_act[1], hidden_dim) for i, obs_act in
                          zip(range(self.agent_n), dim_info.values())}
