...
import random
from ATT import Attention_Critic
    ...
    def learn(self, batch_size, gamma, tau):
        ...
        for agent_id, agent in self.agents.items():
            ...
            entropy_next = - next_log_pi[
                agent_id]  # -torch.sum(next_probs[agent_id] * next_log_pi[agent_id] ,dim=1,keepdim=True)
            ...
            ***注*** MAAC原代码（离散域）及原论文公式的中 相比于SAC,多乘了log_pi,否则不收敛。
            ...
            if self.attention:
                ...
                pol_target = q_pi - v
                actor_loss = (log_pi * (- pol_target - self.alphas[
                ...
                actor_loss = (log_pi * (- q_pi - self.alphas[
                    agent_id].alpha.detach() * entropy)).mean()  # 这里alpha一定要加detach(),因为在更新critic时,计算图被丢掉了
...
def get_env(env_name, env_agent_n=None, continuous_actions=False):
    ...
    if env_agent_n is None:  # 默认环境
        ...
        env = module.parallel_env(max_cycles=25, continuous_actions=continuous_actions, num_good=env_agent_n,
                                  num_adversaries=3)
