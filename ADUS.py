...
'''trick
trick1:ATT-MADDPG
论文:Modelling the Dynamic Joint Policy of Teammates with Attention Multi-agent DDPG 
代码(他人reimplementation)：https://github.com/maohangyu/marl_demo 我在此基础上修改 改版
...
class Agent_U:
    def __init__(self, obs_dim, action_dim, dim_info, actor_lr, critic_lr, device, trick, agent_id):

        self.actor = Actor(obs_dim, action_dim, ).to(device)
        if trick['ATT']:
            self.critic = ATT_critic(dim_info).to(device)  # ATT_critic_raw为原始版本
        else:
...
## 第二部分：定义DQN算法类
class MADDPG:
    def __init__(self, dim_info, is_continue, actor_lr, critic_lr, buffer_size, device, trick):
        self.agents = {}
        self.buffers = {}

        for agent_id, (obs_dim, action_dim) in dim_info.items():
            if agent_id in ('agent_0', 'agent_1', 'agent_2'):
                self.agents[agent_id] = Agent_U(obs_dim, action_dim, dim_info, actor_lr, critic_lr, device, trick, agent_id)
                self.buffers[agent_id] = Buffer(buffer_size, obs_dim, act_dim=action_dim if is_continue else 1,
                                                device=device)
            else:
                self.agents[agent_id] = Agent_S(obs_dim, action_dim, dim_info, actor_lr, critic_lr, device, trick, agent_id)
...
def get_env(env_name, env_agent_n=None):
    ...
    env.reset()
    dim_info = {}
    for agent_id in env.agents[0:3]:
        dim_info[agent_id] = []
    ...
    for agent_id in env.agents[3:4]:
        dim_info[agent_id] = []
        if isinstance(env.observation_space(agent_id), gym.spaces.Box):
            dim_info[agent_id].append(env.observation_space(agent_id).shape[0])
        if isinstance(env.action_space(agent_id), gym.spaces.Box):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--N", type=int, default=4)  # 环境中智能体数量 默认None 这里用来对比设置
    # 共有参数
    ...
    {agent: env.action_space(agent).seed(seed=args.seed) for agent in
     env_agents}  # 针对action复现:env.action_space.sample()
    ...
    while episode_num < args.max_episodes:
        ...
        if step < args.random_steps:  # # 区分环境里的action_ 和训练的action
            ...
            action_ = {agent_id: np.clip(
                action[agent_id] * max_action + args.gauss_scale * np.random.normal(scale=args.gauss_sigma * max_action,
                                                                                    size=dim_info[agent_id][1]),
                -max_action, max_action, dtype=np.float32) for agent_id in env_agents}
            action_ = {agent_id: (action_[agent_id] + 1) / 2 for agent_id in env_agents}  # [-1,1] -> [0,1]

        # 探索环境
        print(episode_num, action_)
        next_obs, reward, terminated, truncated, infos = env.step(action_)
        print('env.step(action_)', env.step(action_))
        ...
        if any(done.values()):
            ...
            if args.gauss_init_scale is not None:
                ...
                args.gauss_scale = args.gauss_final_scale + (
                            args.gauss_init_scale - args.gauss_final_scale) * explr_pct_remaining
            ## 显示
